package com.malim.refresh.weight;
...
import com.malim.refresh.utils.ScrollingUtil;
import com.malim.refresh.weight.Footer.BottomProgressView;
import com.malim.refresh.weight.Footer.IBottomView;
import com.malim.refresh.weight.header.GoogleDotView;
import com.malim.refresh.weight.header.IHeaderView;
import com.malin.utils.DensityUtil;

/**
 * Created by Administrator on 2017/2/17.
 */

public class MalinRefreshLayout extends RelativeLayout {
    //波浪的高度,最大扩展高度
    ...
    public MalinRefreshLayout(Context context) {
        this(context, null);
    }

    public MalinRefreshLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public MalinRefreshLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        //使用isInEditMode解决可视化编辑器无法识别自定义控件的问题
        if (isInEditMode()) return;
        setRefreshListener(new MalinRefreshListener());

        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.MalinRefreshLayout, defStyleAttr, 0);
        mWaveHeight = a.getDimensionPixelSize(R.styleable.MalinRefreshLayout_tr_wave_height, (int) DensityUtil.dp2px(context, 120));
        mHeadHeight = a.getDimensionPixelSize(R.styleable.MalinRefreshLayout_tr_head_height, (int) DensityUtil.dp2px(context, 80));
        mBottomHeight = a.getDimensionPixelSize(R.styleable.MalinRefreshLayout_tr_bottom_height, (int) DensityUtil.dp2px(context, 60));
        mOverScrollHeight = a.getDimensionPixelSize(R.styleable.MalinRefreshLayout_tr_overscroll_height, (int) mHeadHeight);
        enableLoadmore = a.getBoolean(R.styleable.MalinRefreshLayout_tr_enable_loadmore, true);
        isPureScrollModeOn = a.getBoolean(R.styleable.MalinRefreshLayout_tr_pureScrollMode_on, false);
        isOverScrollTopShow = a.getBoolean(R.styleable.MalinRefreshLayout_tr_overscroll_top_show, true);
        isOverScrollBottomShow = a.getBoolean(R.styleable.MalinRefreshLayout_tr_overscroll_bottom_show, true);
        enableOverScroll = a.getBoolean(R.styleable.MalinRefreshLayout_tr_enable_overscroll, true);
        a.recycle();
    ...
    public class CoProcessor {
        ...
        public Context getContext() {
            return MalinRefreshLayout.this.getContext();
        }
